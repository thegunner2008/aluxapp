...
extension StringExtension on String {
  ...
  DateTime toDateTimeFormat(String? format, {String? format2}) {
    if (format == null || format.isEmpty) return toDateTime;
    final inputFormat = DateFormat(format);
    final inputFormat2 = format2 != null ? DateFormat(format2) : null;
    final outputFormat = DateFormat('yyyy-MM-ddTHH:mm:ss');

    String outputString = "";
    try {
      outputString = outputFormat.format(inputFormat.parse(this));
    } catch (e) {
      if (inputFormat2 != null) {
        try {
          outputString = outputFormat.format(inputFormat2.parse(this));
        } catch (e) {
          print(e);
        }
      }
  ...
  String camelCaseToSnakeCase() => splitMapJoin(RegExp(r'([A-Z])'),
      onMatch: (Match match) => '_${match.group(0)?.toLowerCase() ?? ""}',
      onNonMatch: (String nonMatch) => nonMatch.toLowerCase());
}
